  ...
  <div id="app">
          ...
          <div class="" v-for="datas in filter" :key="datas.id">
            <div class="sort" v-if="max >= Number(datas.price)">
              <b-col
                cols="12"
                class="mt-3"
                style="padding-left: 5.5rem; padding-right: 0;"
              >
                <b-card-text>
                  <list
                    :name="datas.name"
    ...
    <b-container>
      <div class="cart">
        <div class="cart-text">
          <p>
            Cart <span class="badge badge-pill badge-info">{{ qty }}</span>
          </p>
        </div>
      </div>
      ...
      <div>
        <b-modal id="checkout" title="Checkout">
          <p class="invoice">invoice : {{ invoice }}</p>
          <div v-for="data of cart" :key="data.id">
            <b-row>
              <b-col>
                <p>
                  {{ data.product.name }}<span> x {{ data.qty }}</span>
                </p>
              </b-col>
              <b-col>
          ...
          <div>
            <p class="tax">Tax 10% : Rp.{{ tax }}</p>
          </div>
          <div>
            <p class="modal-text3">Total : Rp.{{ total }}</p>
          </div>
          <div>
            <p>Cashier : {{ checkout.chasier }}</p>
          </div>
          <b-button block variant="success" @click="addCheckout()"
            >Print</b-button
          >
        </b-modal>
      </div>
...
export default {
  ...
  methods: {
    ...
    addCheckout() {
      ...
      this.checkout.invoice = this.invoice;
      axios({
        method: "POST",
  ...
  computed: {
    ...
    invoice() {
      let idInvoice = "_" + 1 + Math.floor(Math.random() * 999999999);
      return idInvoice;
    },
    tax() {
      let tax = this.calculate * (10 / 100);
      return tax;
    },
    total() {
      const total = this.calculate + this.tax;
      return total;
    },
    ...mapGetters(["getToken"]),
  },
